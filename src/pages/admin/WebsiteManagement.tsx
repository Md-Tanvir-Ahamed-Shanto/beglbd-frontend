...
const WebsiteManagement = () => {
  ...
  const handleHeroUpdate = () => {
    ...
    axios
      .post(
        `${import.meta.env.VITE_BASE_URL}/update_hero_section_data`,
        heroData
      )
      .then(() => {
  ...
  const handleUpadeStats = () => {
    ...
    axios
      .post(`${import.meta.env.VITE_BASE_URL}/add_stats`, statisticsData)
      .then(() => {
    ...
    () => async () => {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/our_services`
        );
  ...
  const handleSaveService = async (serviceFromModal: ServiceDoc) => {
    try {
      if (editingService && (editingService._id || serviceFromModal._id)) {
        ...
        await axios.patch(
          `${import.meta.env.VITE_BASE_URL}/update_service_data/${id}`,
          {
  ...
  const handleDeleteService = async (docIdOrLocalId: string | number) => {
    ...
    if (!idToDelete) {
      setServicesData(
        servicesData.filter((s) => (s._id ?? s.id) !== docIdOrLocalId)
      );
      return;
    }
    try {
      await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/delete_service_data/${idToDelete}`
      );
      await fetchServices();
  ...
  const [editingUniversity, setEditingUniversity] =
    useState<UniversityDoc | null>(null);
  ...
  const fetchUniversities = useMemo(
    () => async () => {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/all_university_partners`
        );
  ...
  const handleSaveUniversity = async (fromModal: UniversityDoc) => {
    try {
      if (editingUniversity && (editingUniversity._id || fromModal._id)) {
        ...
        await axios.patch(
          `${import.meta.env.VITE_BASE_URL}/update_university/${id}`,
          {
            name: fromModal.name,
        ...
        await axios.post(
          `${import.meta.env.VITE_BASE_URL}/add_new_university`,
          {
            name: fromModal.name,
            country: fromModal.country,
            countryCode: fromModal.countryCode,
  ...
  const handleDeleteUniversity = async (docIdOrLocalId: string | number) => {
    const found = universitiesData.find(
      (u) => (u._id ?? u.id) === docIdOrLocalId
    );
    const idToDelete = found?._id;
    if (!idToDelete) {
      // local-only fallback remove
      setUniversitiesData(
        universitiesData.filter((u) => (u._id ?? u.id) !== docIdOrLocalId)
      );
      return;
    }
    try {
      await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/delete_university_data/${idToDelete}`
      );
      await fetchUniversities();
  ...
  // Fetch FAQs on mount
  const fetchFAQs = useMemo(
    () => async () => {
      setFaqLoading(true);
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/all_faqs`
        );
  ...
  const handleDeleteFaq = async (docIdOrLocalId: string | number) => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/delete_faq_data/${idToDelete}`
      );
      await fetchFAQs();
      alert("FAQ deleted!");
    } catch (e) {
  ...
  const fetchContactInfo = useMemo(
    () => async () => {
  ...
  const handleContactInfo = async () => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_BASE_URL}/add_contact_informations`,
        contactData
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setHeroData({ ...heroData, primaryButton: e.target.value })
              }
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setHeroData({ ...heroData, secondaryButton: e.target.value })
              }
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Number of students who have shown interest
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            Number of student files that have been opened
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            Number of students who have successfully flown
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div className="bg-white p-4 rounded-lg border">
            <div className="text-2xl font-bold text-blue-600">
              {statisticsData?.interestedStudents.toLocaleString()}+
            </div>
            <div className="text-sm text-gray-600">Interested Students</div>
          </div>
          <div className="bg-white p-4 rounded-lg border">
            <div className="text-2xl font-bold text-green-600">
              {statisticsData?.filesOpened.toLocaleString()}+
            </div>
            <div className="text-sm text-gray-600">Files Opened</div>
          </div>
          <div className="bg-white p-4 rounded-lg border">
            <div className="text-2xl font-bold text-purple-600">
              {statisticsData?.successfullyDeparted.toLocaleString()}+
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setContactData({ ...contactData, email1: e.target.value })
                }
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setContactData({ ...contactData, email2: e.target.value })
                }
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Clicking will open WhatsApp chat
              </p>
            </div>
          </div>
        </div>
